...
from dataset import LUMINA_BIRADS, MultiImagesDataset, get_fold
...
def load_data(args):
    ...
    image_list, label_list = LUMINA_BIRADS(root = args.data_path)
...
if __name__ == "__main__":
    ...
    for i in range(len(x_name)):
        if 'Benign' in x_name[i][0]:
            ...
            if int(os.path.basename(x_name[i][0]).replace('L_MLO.png', '').replace('L_CC.png', '').replace('R_MLO.png', '').replace('R_CC.png', '')) in low['Malign']:
                ...
                energy_list.append(1)
    y_output = [y>0.5 for y in y_predit]
    y_label_low = []
    y_label_high = []
    y_predit_low = []
    y_predit_high = []
    ...
    low_result['auc'] = roc_auc_score(y_label_low, y_predit_low)
    high_result['auc'] = roc_auc_score(y_label_high, y_predit_high)
    global_result['auc'] = roc_auc_score(y_label, y_predit)
    low_result['f1'] = f1_score(y_label_low, y_output_low)
    high_result['f1'] = f1_score(y_label_high, y_output_high)
    global_result['f1'] = f1_score(y_label, y_output)
